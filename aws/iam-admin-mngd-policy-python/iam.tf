...
resource "aws_iam_user_policy_attachment" "test_attach_user_fail" {
  user       = aws_iam_user.user.name
  policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
}
# Test user policy attachment with other policy - Fail
resource "aws_iam_user_policy_attachment" "test_attach_user_pass" {
  user       = aws_iam_user.user2.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ReadOnlyAccess"
}

# Test role policy attachment with AdministratorAccess - Fail
resource "aws_iam_role_policy_attachment" "test_attach_role_fail" {
  role       = aws_iam_role.role.name
  policy_arn = "arn:aws:iam::aws:policy/AdministratorAccess"
}
# Test group policy attachment with AdministratorAccess - Fail
resource "aws_iam_group_policy_attachment" "test_attach_group_fail" {
